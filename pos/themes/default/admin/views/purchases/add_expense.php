...
<div class="modal-dialog">
    <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            <div class="form-group">
                <?= lang('Expend Type', 'Expend Tyep');?>
                <?php
                    $ct[''] = lang('select').' '.lang('Expend_type');
                    if ($ex_type) {
                       foreach ($ex_type as $tex) {
                          $ct[$tex->id] = $tex->expend_name;
                       }
                    }
                ?>
                <?= form_dropdown('expend_ty', $ct, set_value('Expend Type'), 'class="form-control tip" id="exType"'); ?>
            </div>

            <div class="form-group">
                <?= lang('Supplier', 'Supplier'); ?>
                <?php
                $st[''] = lang('select').' '.lang('supplier');
                if ($suppliers) {
                    foreach ($suppliers as $category) {
                        $st[$category->id] = $category->name;
                    }
                }
                ?>
                <?= form_dropdown('category', $st, set_value('category'), 'class="form-control tip" id="category"'); ?>
            </div>
