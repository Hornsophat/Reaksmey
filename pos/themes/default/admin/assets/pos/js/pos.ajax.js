$(document).ready(function() {
    ...
    $(".open-brands").click(function () {
        $('#brands-slider').toggle('slide', { direction: 'right' }, 1);
    });
    $(".open-category").click(function () {
        $('#category-slider').toggle('slide', { direction: 'right' }, 1);
    });
    $(".open-subcategory").click(function () {
        $('#subcategory-slider').toggle('slide', { direction: 'right' }, 1);
    });
...
$(document).on('change', '#poscustomer', function () {
    ...
    $('#poscustomer').val($(this).val());
});

$(document).on('change', '#checkin_id', function () {
    localStorage.setItem('checkin_id', $(this).val());
    $('#checkin_id').val($(this).val());
});



if (pos_customer = localStorage.getItem('poscustomer')) {
    $('#poscustomer').val(pos_customer);
}
...
$(document).on('change','#cusname',function(){
    localStorage.setItem('cusname',$(this).val());
    $('#cusname').val($(this).val());
});

// if (cusname = localStorage.getItem('cusname')) {
//     $('#cusname').val(cusname);
// }

$(document).on('focus', '#cus_phone', function(){
    localStorage.setItem('cus_phone' , $(this).val());
    $('#cus_phone').val($(this).val());
});

// if (cus_phone = localStorage.getItem('cus_phone')) {
//     $('#cus_phone').val(cus_phone);
// }

$(document).on('change', '#cus_company', function(){
    localStorage.setItem('cus_company', $(this).val());
    $('#cus_company').val($(this).val());
});

if (cus_company = localStorage.getItem('cus_company')) {
...
if (checkin_id = localStorage.getItem('checkin_id')) {
    $('#checkin_id').select2('val', checkin_id);
}
...
function loadItems() {
    ...
    if (localStorage.getItem('positems')) {
        ...
        if (pos_settings.remote_printing != 1) {
            ...
            $("#order_span").empty(); $("#bill_span").empty();$("#order_spansec").empty();
            ...
            var pos_head1 = '';
            var pos_head2 = '</h4><h6><div class="col-md-12 pull-left" style="text-align:left;font-size:15px !important;">លេខតុ: '+$('#select2-chosen-1').text()+'</div></h6></span>';
            $("#order_span").prepend(styles + pos_head1+' '+lang.order+' '+pos_head2);
            $("#order_spansec").prepend(styles + pos_head1+' '+lang.order+' '+pos_head2);
            $("#bill_span").prepend(styles + pos_head1+''+pos_head2);
            $("#order-table").empty(); $("#bill-table").empty();$("#order-tablesec").empty();
        }
        ...
        $.each(sortedItems, function () {
            ...
            var cate_name = item.row.category_name;
            ...
            // item_price = item_tax_method == 0 ? formatDecimal((unit_price), 2) : formatDecimal(unit_price);
            ...
            total += formatDecimal(((parseFloat(item_price) + parseFloat(pr_tax_val)+ item_discount) * parseFloat(item_qty)), 4);
...
// ======================================================= add  new for looping row ================================================;
                ...
                var bprTr = '<tr style="font-size:14px !important;line-height:25px !important;border-top:dotted 2px !important;" class="row_' + rows + '" data-item-id="' + rows + '"><td colspan="" style="text-align:center !important;">'+ (an-1) + '</td>';               
                bprTr += '<td style="white-space: nowrap;font_style:bold;">'+ item_name +'</td>';
                bprTr += '<td style="text-align:center !important;white-space: nowrap; font_style:bold;">'+ formatDecimal(item_qty)+munit[product_unit] +'</td>';
                bprTr += '<td style="white-space: nowrap;font_style:bold;"> $ '+Number(parseFloat(item_price)+ parseFloat(pr_tax_val) + item_discount).toFixed(2) +'</td>';

                if(item_discount == 0) {
                    bprTr += '<td style="text-align:center !important;font_style:bold;"></td>';
                } else if((formatMoney(((parseFloat(item_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty)))) == 0) {
                    bprTr += '<td style="text-align:center !important;font_style:bold;">Free</td>';
                } else {
                    // bprTr += '<td style="text-align:center !important;"> $ &nbsp;'+ formatDecimal(item_qty)*item_discount +'</td>';
                    bprTr += '<td style="text-align:center !important;font_style:bold;">'+ formatDecimal(ds)+'&nbsp; % &nbsp;('+formatDecimal(item_qty)*item_discount+'$)</td>';
                ...
                bprTr += '<td style="text-align:right;font_style:bold;">$ '+Number( ((parseFloat(item_price) + parseFloat(pr_tax_val) + item_discount) * parseFloat(item_qty))).toFixed(2)+' &nbsp;</td></tr>';
                // rows++;
                if(cate_name == 'Breakfast' || cate_name == 'Lunch' || cate_name =='Dinner'){
                var oprTr = '<tr " class="row_' + item_id + '" data-item-id="' + item_id + '"><td style="font-size:12px !important;line-height:20px !important;">#'+(an-1)+' ' + item_name + ' (' +(sel_opt != '' ? sel_opt : '')+ ')</td><td>' + formatDecimal(item_qty) +'</td></tr>';
                }else if(cate_name == 'Drink'){
                var oprTrsec = '<tr " class="row_' + item_id + '" data-item-id="' + item_id + '"><td style="font-size:12px !important;line-height:20px !important;">#'+(an-1)+' ' + item_name + ' (' +(sel_opt != '' ?sel_opt : '')+')</td><td>' + formatDecimal(item_qty) +'</td></tr>';
                }
                $("#bill-table").append(bprTr);

                $("#order-table").append(oprTr);

                $("#order-tablesec").append(oprTrsec);
                
            }
        });
// ======================================add new for looping row table=========================================================
    // var bprTrr;
    //     for(cell=rows; cell<=9; cell++){

    //                 bprTrr+="<tr>";
    //                     bprTrr +="<td style='text-align:center !important;font-size:11px;'>"+cell+"</td>";

    //                     bprTrr+="<td style='border-right:1px solid #ddd;'></td>";
    ...
    //                     bprTrr+="<td style='border-right:1px solid #ddd;'></td>                     </tr>";
    //             }
...
// ======================================add new for looping row table=========================================================
        ...
        var gtotal = parseFloat(((total + invoice_tax) -total_discount) + parseFloat(shipping));
        console.log(gtotal);
        ...
        if (pos_settings.remote_printing != 1) {
            ...
            var bill_totals = '';
            bill_totals += '<tr class=" fitem"><td style="text-align:center;border-top:2px dotted #ddd;font-size:12px;white-space: nowrap;" rowspan=4>អត្រាប្តូរប្រាក់​ :'+Number(rate_cur)+'(R) </br> </td><td style="border-top:2px dotted #ddd; font-size:14px;">សរុបដុល្លា</td><td style="font-size:14px !important;text-align:right; border-top:2px dotted  #ddd">$ '+Number(total).toFixed(2)+'&nbsp; </td></tr>';
            
            if(order_discount > 0 || product_discount > 0) {
                bill_totals += '<tr class=" fitem"><td style="font-size:12px;">បញ្ចុះតំលៃ</td><td style="font-size:14px !important;text-align:right;">$ '+Number(order_discount+product_discount).toFixed(2)+' &nbsp;</td></tr>';
            }
            if (site.settings.tax2 != 0 && invoice_tax != 0) {
                bill_totals += '<tr class=" fitem"><td>'+lang.order_tax+'</td><td style="font-size:14px !important;text-align:right;">R '+Number(invoice_tax).toFixed(2)+'&nbsp;</td></tr>';
            }
            ...
            bill_totals += '<tr class=" fitem"><td style="font-size:14px;">សរុបចុងក្រោយ </td><td style="font-size:14px !important;text-align:right;font-weight:bold;">$ '+Number(gtotal).toFixed(2)+' &nbsp;</td></tr>';
            bill_totals += '<tr class=" fitem"><td style="font-size:14px;">សរុបរៀល </td><td style="font-size:14px !important;text-align:right;font-weight:bold;">រៀល  '+Number(gtotal*rate_cur).toFixed(0)+' &nbsp;</td></tr>';
            if(pos_settings.rounding != 0) {
                round_total = roundNumber(gtotal, parseInt(pos_settings.rounding));
                var rounding = formatDecimal(round_total - gtotal);
                bill_totals += '<tr class=" fitem"><td>'+lang.rounding+'</td><td style="font-size:14px !important; text-align:right;border-top:1px dotted #000 !important;">'+Number(rounding).toFixed(2)+' &nbsp;</td></tr>';
                bill_totals += '<tr class=" fitem"><td>'+lang.total_payable+'</td><td style="font-size:18px !important; text-align:right;border-top:1px $dotted #000 !important;">'+Number(round_total).toFixed(2)+' &nbsp;</td></tr>';
            }
