...
class Companies_model extends CI_Model
{
    ...
    //     if($this->db->insert('sma_table',$data)){
    ...
    public function getTableSuggestion($term, $limit = 10){
        $this->db->select("id,(CASE WHEN company = '-' THEN name ELSE CONCAT(company, ' (',name, ')') END) as text", FALSE);
        $this->db->where(" (id LIKE '%" . $term . "%' OR table LIKE '%" . $term . "%' OR company LIKE '%" . $term . "%' OR email LIKE '%" . $term . "%' OR phone LIKE '%" . $term . "%') ");
        $this->db->where('table IS NOT NULL');
        $q = $this->db->get_where('companies', array('group_name' => 'table','group_name'=>'customer'), $limit);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
