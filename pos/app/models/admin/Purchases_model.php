...
class Purchases_model extends CI_Model
{
    ...
    public function getProductNames($term,$category_id,$limit = 0)
    {
    ...
    public function save_reco_payment($save_data){
        if($this->db->insert('sma_payment_record',$save_data)){
    ...
    public function getAllExpendBySupplier($id){
        $re="SELECT *,c.name,c.company,p.grand_total as amount 
    ...
    public function addExpenseBySubpp($data = array()){
      if ($this->db->insert('purchases',$data)) {
          return true;
      }
      return false;
    ...
    public function updateExpenseBysupp($id,$data = array()){
    ...
    public function deleteExpensePay($id){
    ...
    function supplier_receive_payment($supp_id,$fd,$ld){
        // $where_supp = '';
        // if ($where_supp !='') {
        //     $where_supp = "supplier_id = $supp_id";
        // }else{
        //     $where_supp=''
        // }
        $sup="SELECT * ,sum(grand_total) as gtotal,sum(paid) as paid_total FROM sma_purchases
            WHERE supplier_id = $supp_id AND payment_status <> 'paid'
            GROUP BY id 
            ORDER BY id desc";

        // $re=$this->db->query($sup)->result();
        // var_dump($re);die();
        return $this->db->query($sup)->result();
    }

    public function save_record_detail($data){
        $this->db->insert('sma_payment_record_detail',$data);
    }
     public function addExpendType($data = array()){
        if ($this->db->insert('expent_type',$data)) {
            return true;
        }
        return false;
    }
    function getAllPurchaserecieve($id){
         $q = $this->db->get_where('sma_purchase_receive', array('purchase_id' => $id));
        if ($q->num_rows() > 0) {
            return $q->result();
        }
        return FALSE;
