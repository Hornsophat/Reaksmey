...
class Site extends CI_Model
{
    ...
    public function __construct() {
        ...
        //     echo "Please Contact to system Administrator : 0969946219 "; die();
        // }
        // password on serial 
        // if(trim($serial)!='1EBE-FA91'){
        //     echo "Please Contact to system Administrator : 0969946219 "; die();
        // }
    }
    ...
    public function getAllWarehouses() {
        $sp_user = $this->session->userdata();
        if ($this->Owner || $this->Admin) {
            $q = $this->db->get('warehouses');
        }else{
            $q = $this->db->get_where('warehouses',array('id'=>$sp_user['warehouse_id']));
        }
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
    ...
    public function getAllCategories() {
        $this->db->where('parent_id', NULL)->or_where('parent_id', 0)->order_by('sname');
     ...
     public function getAllSubcategories($cate_id=null) {
        ...
        if ($q->num_rows() > 0) {
            ...
            switch ($field) {
                ...
                case 'ppay':
                    ...
                    break;
                case 'ex':
                    $prefix = isset($this->Settings->expense_prefix) ? $this->Settings->expense_prefix . 'EXP' : '';
                    break;
                case 're':
            ...
            if ($this->Settings->reference_format == 1) {
                ...
                $ref_no .= date('Y') . "/" . date('m') . "/" . sprintf("%06s", $ref->{$field});
    ...
    public function getAllTables($fl){
        $this->db->select("companies.*,customer_id");
        $this->db->from('companies');
        $this->db->join('suspended_bills','suspended_bills.customer_id = companies.id','left');
        $this->db->where('group_name','table');
        $this->db->where('tbl_location',$fl);
        $re = $this->db->get();
        if ($re->num_rows() > 0) {
            return $re->result();
        }
    ...
    public function checkSlug($slug, $type = NULL) {
     ...
     public function getRecordpayment(){
        return $this->db->query("SELECT pr.rec_id,c.company,pr.date,pr.amount_usd,pr.amount_r,pr.amount_b FROM sma_payment_record pr
                ...
                WHERE pr.delete=0 AND c.group_name='customer'
                order by c.company='desc'")->result();
    }
    public function getRecordpurchasespayment(){
        return $this->db->query("SELECT pr.rec_id,c.company,pr.date,pr.amount_usd,pr.amount_r,pr.amount_b FROM sma_payment_record pr
                INNER JOIN sma_companies c ON pr.cus_id=c.id
                WHERE pr.delete=0 AND c.group_name='Supplier'
                order by c.company='desc'")->result();
    }

    function get_amount_remain(){
        return $this->db->query("SELECT sum(grand_total) from sma_sales where payment_status<>'paid'")->result();
    }
    ...
    function getAllPaymentByCus($cus_id,$df,$dt){
        ...
        return $this->db->query("SELECT * FROM sma_sales WHERE customer_id = $cus_id AND payment_status<>'paid' AND paid < grand_total {$where}")
                        ->result();
        // print_r($q);die();
    }
    ...
    public function get_selected_purcheases($supp_id,$purchase_id){
        $where =' AND(';
        for ($i=0; $i < count($purchase_id) ; $i++) { 
                $where.="id='".$purchase_id[$i]."'";
                
                if($i<count($purchase_id)-1)
                    $where.=' OR ';
        }
        $where.=')';
        return $this->db->query("SELECT * FROM sma_purchases WHERE supplier_id = $supp_id AND payment_status<>'paid' {$where}")
                        ->result();
    }

    public function getRecordpaymentByid($id){
        return $this->db->query("SELECT c.phone,c.address,pr.rec_id,c.company,pr.date,pr.amount_usd,pr.amount_r,pr.amount_b,pr.billers,pr.cus_id FROM sma_payment_record pr
                                        INNER JOIN sma_companies c ON pr.cus_id=c.id
                                        WHERE pr.delete=0 AND c.group_name='customer' AND pr.rec_id='$id'")->row();
    }
    public function getRecordpaymentByid_purchases($id){
        return $this->db->query("SELECT c.phone,c.address,pr.rec_id,c.company,pr.date,pr.amount_usd,pr.amount_r,pr.amount_b,pr.billers,pr.cus_id FROM sma_payment_record pr
                                        INNER JOIN sma_companies c ON pr.cus_id=c.id
                                        WHERE pr.delete=0 AND c.group_name='Supplier' AND pr.rec_id='$id'")->row();
    }
    public function get_RecDetail($rec_id){
        return $this->db->query("SELECT * FROM sma_payment_record_detail rd 
                                INNER JOIN sma_sales s ON rd.sale_id=s.id
                                WHERE rec_id = $rec_id AND (rd.amount_usd>0 OR rd.amount_b>0)")->result();
    }
    // function get_remain_payment($rec_id){
    //     return $this->db->query("SELECT * FROM sma_sales where customer_id='$rec_id' AND payment_status<>paid");
    // }
    ...
    function getCusSale($cus_id){
        $sql = $this->db->query("SELECT SUM(grand_total) as gtotal, SUM(paid) as paid FROM sma_sales WHERE customer_id='$cus_id'");

        return $sql->row();
    }

    function getpurchases($cus_id){
        $sql = $this->db->query("SELECT SUM(grand_total) as gtotal, SUM(paid) as paid FROM sma_purchases WHERE supplier_id='$cus_id'");

        return $sql->row();
    }
    function getHotelCustomer(){
        $this->load->database();
        $db2 = $this->load->database('c855sys7_dnhotel',TRUE);

        // $q = $db2->select('date_in , date_out')->get('tbl_checkin');
        // $qu = $q->row_array();
        $get_cusHotelCheckout = $this->db->query("
            SELECT tcu.id,tcu.Name,tcu.Family,tcu.Mobile,tcu.Company
            FROM c855sys7_dnhotel.tbl_checkin tch
            LEFT JOIN c855sys7_dnhotel.tbl_customer tcu 
            ON tcu.id = tch.customer_id
            WHERE tch.checkouted = 0
            ")->result();
        return $get_cusHotelCheckout;
    }
    function getHotelCheckinRooms(){
       $this->load->database();
        $db2 = $this->load->database('c855sys7_dnhotel',TRUE);
        
        // $rooms = $db2->query("SELECT r.*,c.id as checkin_id,c.customer_id FROM tbl_checkin c
        //                             INNER JOIN tbl_room r ON c.room_no = r.id
        //                             WHERE checkouted = 0")->result();
        $rooms = $db2->query("SELECT
                            checkin.*,checkin.id AS checkin_id,chek_de.room_no
                            FROM
                                tbl_checkin_detail chek_de
                            INNER JOIN tbl_checkin checkin ON chek_de.checkin_id = checkin.id
                            WHERE
                            chek_de.room_id IS NOT NULL
                            AND checkin.checkouted = 0
                            GROUP BY checkin.id")->result();

        return $rooms;
    }

    function getHotelCustomerById($cus_id){
        $getHCVID = $this->db->query("SELECT * FROM c855sys7_dnhotel.tbl_customer WHERE id ='".$cus_id."'")->row();
        return $getHCVID;
    }
