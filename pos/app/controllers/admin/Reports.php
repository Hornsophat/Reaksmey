...
class Reports extends MY_Controller
{
    ...
    function userDailysale(){
        $uid = $this->session->userdata('user_id');
        $openDate = $this->db->query("SELECT `date` FROM sma_pos_register WHERE user_id = '".$uid."' AND status ='open'")->row()->date;
        $this->data['udaily_sale'] = $this->getDailySale($openDate,$uid);

        $this->data['people']=$this->db->where('group_name','customer')->get('companies')->result();
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('userDaily')));
        $meta = array('page_title' => lang('userDaily'), 'bc' => $bc);
        $this->page_construct('reports/account/userDaily', $meta, $this->data);
    }

    public function customerDailySale(){
        $customerId = $this->input->get('customer');
        $uid = $this->session->userdata('user_id');
        $openDate = $this->db->query("SELECT `date` FROM sma_pos_register WHERE user_id = '".$uid."' AND status ='open'")->row()->date;
        // var_dump($customerId);die();
        $this->data['customerrow'] = $customerId;
        $this->data['udaily_sale'] = $this->getCustomerTime($customerId,$openDate,$uid);
        $this->data['people']=$this->db->where('group_name','customer')->get('companies')->result();
    ...
    function getCustomerTime($customerId,$openDate,$uid){
        $where = '';
        if ($customerId != '') {
            $where = 'AND customer_id = "'.$customerId.'"';
        }
        $sql = $this->db->query("SELECT ss.date as sdate,ss.reference_no,ss.customer as sale_table,ss.total
        FROM sma_sales ss
        LEFT JOIN sma_pos_register pr
        ON pr.user_id = ss.created_by
        WHERE ss.date >= '".$openDate."' AND pr.`status` = 'open' AND ss.created_by = '".$uid."' {$where}")->result();
        return $sql;
    }

    function getDailySale($openDate,$uid){
        $sql = $this->db->query("SELECT ss.date as sdate,ss.reference_no,ss.customer_id,ss.customer as sale_table,s.product_name,s.quantity,s.real_unit_price,SUM(s.subtotal) as subtotal
        FROM sma_sales ss
        LEFT JOIN sma_pos_register pr
        ON pr.user_id = ss.created_by
        LEFT JOIN sma_sale_items s 
        ON s.sale_id = ss.id
        WHERE ss.date >= '".$openDate."' AND pr.`status` = 'open' AND ss.created_by = '".$uid."' 
        GROUP BY s.product_code,ss.customer_id
        ORDER BY ss.customer_id DESC")->result();
         // var_dump($sql);die();
    ...
    function customeers_transaction(){
        ...
        $warehouse_id = isset($_GET['warehouse_all'])?$_GET['warehouse_all']:'';
        // var_dump($warehouse_id);die();
        // $product_idstr = '';
        // $product_arr = array();
        // for ($i=0; $i < count($product_id); $i++) { 
        //     $product_arr[$product_id[$i]] = $product_id[$i];
        //     $product_idstr .=",".$product_id[$i]; 
        // }
        // $product_idstr = trim($product_idstr,',');
        // echo $product_idstr;
        ...
        $this->data['people']=$this->db->where('group_name','customer')->get('companies')->result();
        $this->data['warehouse_all'] = $this->site->getAllWarehouses();
        $this->data['ware_id'] = $warehouse_id;
        if($category_id!=''){
            $this->db->where('category_id',$category_id);
            $this->data['products']=$this->db->get('products')->result();

        }
        $this->data['customer'] = $this->reports_model->search_customer_buy($fdate,$ldate,$customer_id,$product_from,$warehouse_id,$product_to);
    ...
    //     $this->data['fdate']=$fdate;
    //     $this->data['tdate']=$ldate;
    //     $whid=$this->input->post('ware_id');
    //     $this->data['wares_id']=$whid;
    ...
    //     $this->data['cate'] = $this->site->getAllcategories();
    //     $this->data['scate'] = $this->site->getAllsubcategories($cate);
    //     $this->data['stock'] = $this->reports_model->get_report_stock($whid,$fdate,$ldate,$cate,$scate);
    //     // var_dump($this->data['stock']);die();
    //     $this->data['warehouse']= $this->site->getAllWarehouses();
    ...
    function purchase_items(){
        ...
        $this->data['puchses'] = $this->Reports_model->GetAllpurchaseItems($first_date, $last_date, $category);
        // var_dump($this->data['getrp']);die();
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['categories'] = $this->site->getAllCategories();
        ...
        $meta = array('page_title' => 'Products Items Report', 'bc' => $bc);
        $this->page_construct('reports/account/purchase_items', $meta, $this->data);
    }
    function cash_flow(){
        $date = date("Y-m-d");
        $first_date = $this->sma->fld(trim($this->input->get('first_date')));
        $last_date = $this->sma->fld(trim($this->input->get('last_date')));
        $category = $this->input->get('category');
        $this->load->model('Reports_model');
        $this->data['categories'] = $this->site->getAllCategories();
        
        if ($first_date != '' && $last_date != '') {
            // $this->data['revenu'] = $this->getAllIncome($date,$first_date,$last_date,$category);
            // $this->data['cost'] = $this->getAllCostIncome($date,$first_date,$last_date,$category);
            $this->data['expense'] = $this->getAllExpense($date,$first_date,$last_date,$category);
            $this->data['purchase'] = $this->reports_model->getAllPurchases($first_date, $last_date, $category);
            // $this->data['return'] = $this->getAllReturnt($date,$first_date,$last_date,$category);
            // $this->data['shipping'] = $this->getAllShipping($date,$first_date,$last_date,$category);
            // $this->data['payment'] = $this->getAllPayment($date,$first_date,$last_date,$category);
            // $this->data['sell'] = $this->getAllSell($date,$first_date,$last_date,$category);
            // $this->data['other_income'] = $this->reports_model->getOtherIncome($first_date, $last_date); // grt all other income
             $this->data['delivery'] = $this->reports_model->getAllTotalDelivery($first_date, $last_date); // grt all total sale
             $this->data['due'] = $this->reports_model->getAllTotalDue($first_date, $last_date); // grt all total sale
             $this->data['paid'] = $this->reports_model->getAllTotalPaid($first_date, $last_date); // grt all total sale
        }
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('custom_report')));
        $meta = array('page_title' => 'Expenses Report', 'bc' => $bc);
        $this->page_construct('reports/account/cashflow', $meta, $this->data);
    }
     function getAllExpense($date,$first_date,$last_date,$categories){
        $sql = "
        SELECT xp.reference_no,SUM(xp.grand_total) as totalExpense,xpt.expend_name
        FROM sma_purchases xp
        LEFT JOIN sma_expent_type xpt
        ON xpt.id = xp.exType_id
        WHERE xp.status='expend' AND date(xp.`date`) >= '$first_date' AND date(xp.`date`) <= '$last_date'
        GROUP BY xpt.id
        ORDER BY xp.id DESC";
