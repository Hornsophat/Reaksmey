...
class Customers extends MY_Controller
{
    ...
    function list_table($action= null){
       $this->sma->checkPermissions();
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('customers')));
        $meta = array('page_title' => lang('customers'), 'bc' => $bc);
        $this->page_construct('customers/list_table', $meta, $this->data);
    }
    function get_table(){
         $this->sma->checkPermissions('index');
        $this->load->library('datatables');
        $this->datatables
            ->select("id,table,tbl_location")
            ->from("companies")
            ->where('group_name', 'table')
            ->where('table is NOT NULL')
            ->add_column("Actions", "<div class=\"text-center\"><a class=\"tip\" title='" . lang("list_deposits") . "' href='" . admin_url('customers/deposits/$1') . "' data-toggle='modal' data-target='#myModal'><a class=\"tip\" title='" . lang("edit_table") . "' href='" . admin_url('customers/edit_table/$1') . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang("delete_table") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('customers/delete/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", "id");
        //->unset_column('id');
        echo $this->datatables->generate();
    }
    ...
    function add_table(){
       $this->sma->checkPermissions(false, true);
        ...
        if ($this->form_validation->run('companies/add_table') == true) {
            $cg = $this->site->getCustomerGroupByID($this->input->post('customer_group'));
            // $pg = $this->site->getPriceGroupByID($this->input->post('price_group'));
            //Check whether user upload picture
            // if(!empty($_FILES['tableImg']['name'])){
            //     $config['upload_path'] = 'table/';
            //     $config['allowed_types'] = 'jpg|jpeg|png|gif';
            //     $config['file_name'] = $_FILES['tableImg']['name'];
                
            //     //Load upload library and initialize configuration
            //     $this->load->library('upload',$config);
            //     $this->upload->initialize($config);
                
            //     if($this->upload->do_upload('tableImg')){
            ...
            $data = array('name' => $this->input->post('name'),
                ...
                'cf6' => $this->input->post('cf6'),
                'image' => $pictures,
                'tbl_location' => $this->input->post('floor'),
            );
// =================================upload table=====================================================
        ...
        if ($this->form_validation->run() == true && $cid = $this->companies_model->addCompany($data)) {
            ...
            $this->data['customer_groups'] = $this->companies_model->getAllCustomerGroups();
            $this->data['price_groups'] = $this->companies_model->getAllPriceGroups();
            $this->load->view($this->theme . 'customers/add_table', $this->data);
        }
    }
    ...
    function edit_table($id = NULL){
        ...
        if ($this->form_validation->run('companies/edit_table') == true) {
            $cg = $this->site->getCustomerGroupByID($this->input->post('customer_group'));
            $pg = $this->site->getPriceGroupByID($this->input->post('price_group'));
            $data = array('name' => $this->input->post('name'),
                'table' => $this->input->post('name'),
        ...
        if ($this->form_validation->run() == true && $this->companies_model->updateCompany($id, $data)) {
            ...
            $this->data['customer_groups'] = $this->companies_model->getAllCustomerGroups();
            $this->data['price_groups'] = $this->companies_model->getAllPriceGroups();
            $this->load->view($this->theme . 'customers/edit_table', $this->data);
        }
    }
    ...
    function suggestionsTable($term = NULL, $limit = NULL){
        if ($this->input->get('term')) {
           $term = $this->input->get('term', TRUE);
        }
        if (strlen($term) < 1) {
        ...
        $limit = $this->input->get('limit', TRUE);
        $rows['results'] = $this->companies_model->getTableSuggestion($term, $limit);
        // var_dump($rows['results']);die();
        $this->sma->send_json($rows);
    }
    function gettableByID($id){
       return $this->db->query("SELECT * FROM sma_companies WHERE 'group_name'='table' AND id='".$id."'")->result();
    }
    ...
    function getCustomer($id = NULL)
    {
        ...
        // $row = $this->gettableByID($id);

        $this->sma->send_json(array('id' => $row->id, 'text' => ($row->name != '-' ? $row->name : $row->name),'image' => $row->image));
    }
