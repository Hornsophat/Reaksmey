...
class Purchases extends MY_Controller
{
    ...
    public function getPurchases($warehouse_id = null)
    {
        ...
        $receive_link = anchor('admin/purchases/recieve/$1','<i class="fa fa-file-text-o"></i>'.lang('Receive_Data'),'data-toggle="modal" data-target="#myModal"','class="re_disable"');
        ...
        <ul class="dropdown-menu pull-right" role="menu">
            <li>' . $detail_link . '</li>
            <li class="disab">' . $receive_link.'</li>
            <li>' . $payments_link . '</li>
    ...
    function clear_payment(){
        ...
        $this->data['supplier'] = $this->purchases_model->get_all_supplier();
        $this->data['billers'] = $this->site->getAllCompanies('biller');
        // var_dump($this->data['biller']);die();
        ...
        $this->page_construct('purchases/clear_payment', $meta, $this->data);
    }
    public function get_all_supplier_by_ID(){
         // var_dump($_GET);die();
        $supp_id = $this->input->get('supp_id');
        $fd = $this->input->get('date_from');
        $ld = $this->input->get('date_to');

        $all_supplier = $this->purchases_model->supplier_receive_payment($supp_id,$fd,$ld);
        // var_dump($all_supplier);die();
        $tdata='';
        $total_remain=0;
        $i=0;
        foreach ($all_supplier as $sup_pay) {
           $i++;
           $tdata.="<tr>";
                $tdata.="<td><input type='checkbox' class='form-control ch_purchese' name='ch_purchese[]' value='$sup_pay->id' usd='".($sup_pay->gtotal-$sup_pay->paid_total)."'></td>";
                $tdata.="<td>$i</td>";
                $tdata.="<td>$sup_pay->reference_no</td>";
                $tdata.="<td>".date('d/m/Y H:i:s',strtotime($sup_pay->date))."</td>";
                $tdata.="<td>".($sup_pay->gtotal-$sup_pay->paid_total)." $</td>";

           $tdata.="</tr>";

           $total_remain+=($sup_pay->gtotal-$sup_pay->paid_total);
        }

        $tfoot="<tr><th colspan='4' style='text-align:right'>Total</th>";
        $tfoot.="<th>".number_format($total_remain,2)." $</th></tr>";

        // var_dump($tdata);die();

        $this->data['tbody']=$tdata;
    ...
    public function save_payment(){
        ...
        $supp_id = $this->input->get('supp_id');
        $tam_usd = $this->input->get('amount_usd');
        $fd = $this->input->get('date_from');
        $td = $this->input->get('date_to');
        $purchase_id = $this->input->get('purchases_id');
        $amount_usd = 0;
        $date = date('Y-m-d');

        $purchases_pay = $this->site->get_selected_purcheases($supp_id,$purchase_id);

        $save_data=array(
                'cus_id'=>$supp_id,
                'date'=>$date,
                'amount_usd'=>$this->input->get('amount_usd'),
                'billers'=>$this->input->get('billers'),
                'delete'=>0
            );
        $rec_id = $this->purchases_model->save_reco_payment($save_data);

         foreach ($purchases_pay as $val) {
            $amount_usd = 0;
            // $amount_r = 0;
            // $amount_b = 0;
            $purchase_id = $val->id;
            $balance_usd = ($val->grand_total - $val->paid);
            ...
            $payment = array(
                ...
                // 'amount' => 0,
                'amount'=>$this->sma->formatDecimal($amount_usd),
                'paid_by' =>'cash',
                'cc_type' => 'Visa',
                'note' => 'from list sale payment',
                'created_by' => $this->session->userdata('user_id'),
                'type' => 'received'
            );
            if($this->purchases_model->addPayment($payment)){
                $recdetail = array(
                    'rec_id'=>$rec_id,
                    'sale_id'=>$purchase_id,
                    'amount_usd'=>$this->sma->formatDecimal($amount_usd),
                    'payment_status'=>$status
                            );
                $this->purchases_model->save_record_detail($recdetail);
                $this->data['d'] = 'Payment Success';
                $this->data['rec_id'] = $rec_id;
                // echo json_encode($this->data);
            }
        }
    ...
    function print_payment(){
        $this->data['record'] = $this->site->getRecordpurchasespayment();
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('purchases'), 'page' => lang('purchases')), array('link' => '#', 'page' => lang('list_sale_payment')));
        $meta = array('page_title' => lang('list_sale_payment'), 'bc' => $bc);
        $this->page_construct('purchases/print_payment', $meta, $this->data);
    }

    function get_print_payment($id){
        $this->sma->checkPermissions();

        $this->data['rate']=$this->site->getrielcurrency();
        $this->data['voucher'] = $this->site->getRecordpaymentByid_purchases($id);
        $this->data['biller'] = $this->site->getCompanyByID($this->data['voucher']->billers);
        $this->data['balance'] = $this->site->getpurchases($this->data['voucher']->cus_id);
        $this->data['billers'] = $this->site->getAllCompanies('biller');

        $this->data['voucher_detail'] = $this->site->get_RecDetail($id);
        // var_dump($this->data['voucher_detail']);die();
        $this->load->view($this->theme . 'purchases/print_purchases_pay', $this->data);
    }
...
//generate pdf and force to download
    ...
    public function add($quote_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $abroad_date = $this->sma->fld(trim($this->input->post('shipping_date')));
            $abroad_shipping = $this->input->post('abroad_shipping');
            ...
            $grand_total = $this->sma->formatDecimal(($total + $total_tax + $this->sma->formatDecimal($shipping)+ $this->sma->formatDecimal($abroad_shipping) - $order_discount), 4);
            $data = array('reference_no' => $reference,
                ...
                'abroad_shipping' => $abroad_shipping,
    ...
    public function suggestions()
    {
        ...
        $rows = $this->purchases_model->getProductNames($term,$category_id);
        // print_r($rows);die();
        if ($rows) {
    ...
    /* -------------------------------------------------------------------------------- */
     function get_expend_type(){
        return $this->db->query("SELECT * FROM sma_expent_type")->result();
    }
    ...
    function add_expense_type(){
        ...
        $this->form_validation->set_rules('Typename', lang("Typename"), 'required');
        $this->form_validation->set_rules('userfile', lang("attachment"), 'xss_clean');
        if ($this->form_validation->run() == true) {
            if ($this->Owner || $this->Admin) {
            ...
            $data = array(
                ...
                // 'reference' => 'test',
                'expend_name' => $this->input->post('Typename'),
                // 'created_by' => $this->session->userdata('user_id'),
                'expend_description' => $this->input->post('note', true),
            );
            if ($_FILES['userfile']['size'] > 0) {
                $this->load->library('upload');
        ...
        } elseif ($this->input->post('add_expense_type')) {
            $this->session->set_flashdata('error', validation_errors());
            redirect($_SERVER["HTTP_REFERER"]);
        }

        if ($this->form_validation->run() == true && $this->purchases_model->addExpendType($data)) {
            $this->session->set_flashdata('message', lang("expense_added"));
            // redirect($_SERVER["HTTP_REFERER"]);
            redirect('admin/purchases/list_expense_type');
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            ...
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'purchases/add_expend_type', $this->data);
        }
    }
    function get_allExpend(){
        $re="SELECT * from sma_expent_type";
        return $this->db->query($re)->result();
    }
    function list_expense_type(){
        $this->sma->checkPermissions('return_purchases');
        $this->data['list_expent']=$this->get_allExpend();
        // var_dump($this->data['list_expent']);die();
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('purchases'), 'page' => lang('purchases')), array('link' => '#', 'page' => lang('return_purchase')));
            ...
            $this->page_construct('purchases/list_expend_type', $meta, $this->data);
    }
    function deleteEx($id){
        $re=$this->db->where('id',$id)->delete('sma_expent_type');
        if ($re) {
            redirect(admin_url('purchases/list_expense_type'));
        }else{
            redirect(admin_url('purchases/list_expense_type'));
        }
    }
    ...
    public function getExpenses()
    {
        ...
        $this->datatables
            ->select($this->db->dbprefix('purchases') . ".id as id,{$this->db->dbprefix('purchases')}.date, reference_no,{$this->db->dbprefix('expent_type')}.expend_name,{$this->db->dbprefix('companies')}.name as category, grand_total as amount, note, CONCAT({$this->db->dbprefix('users')}.first_name, ' ', {$this->db->dbprefix('users')}.last_name) as user, attachment", false)
            ->from('purchases')
            ->join('users', 'users.id=purchases.created_by', 'left')
            ->join('companies', 'companies.id=purchases.supplier_id', 'left')
            ->join('expent_type','expent_type.id=purchases.exType_id','left')
            ->where('purchases.status','expend')
    ...
    public function add_expense()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                'date' => $date,
                'reference_no' => $this->input->post('reference') ? $this->input->post('reference') : $this->site->getReference('ex'),
                // 'reference' => 'test',
                'grand_total' => $this->input->post('amount'),
                'created_by' => $this->session->userdata('user_id'),
                'note' => $this->input->post('note', true),
                'supplier_id' => $this->input->post('category', true),
                'warehouse_id' => $this->input->post('warehouse', true),
                'exType_id' =>  $this->input->post('expend_ty',true),
                'status' => 'expend',
            );
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addExpenseBySubpp($data)) {
            $this->session->set_flashdata('message', lang("expense_added"));
            redirect($_SERVER["HTTP_REFERER"]);
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            ...
            $this->data['categories'] = $this->purchases_model->getExpenseCategories();
            $this->data['suppliers'] = $this->site->getAllCompanies('supplier');
            $this->data['ex_type'] = $this->get_expend_type();
            // var_dump($this->data['ex_type']);die();
            $this->data['modal_js'] = $this->site->modal_js();
    ...
    public function edit_expense($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->updateExpenseBysupp($id, $data)) {
            ...
            $this->data['expense'] = $this->purchases_model->getAllExpendBySupplier($id);
            $this->data['ex_type'] = $this->get_expend_type();
            // var_dump($this->data['ex_type']);die();
            $this->data['warehouses'] = $this->site->getAllWarehouses();
            $this->data['modal_js'] = $this->site->modal_js();
    ...
    public function delete_expense($id = null)
    {
        ...
        $expense = $this->purchases_model->getAllExpendBySupplier($id);
        if ($this->purchases_model->deleteExpensePay($id)) {
    ...
    public function update_status($id)
    {
        ...
        // $re_date = $this->input->post('receive_date');
        // $this->db->insert('');
    ...
    function recieve($id = NULL)
    {
        ...
        $no=$this->db->query("SELECT max(no) as no FROM sma_purchase_receive")->row()->no;
        $inv_no='RC'.date('Ym-').sprintf('%04d', $no+1);
        $row=$this->db->where('id',$id)->get('purchases')->row();
        $this->data['rows'] = $row->status=='received' ? array() :$this->purchases_model->getAllPurchaseItems($id);
        $this->data['pu'] = $this->purchases_model->getPurchaseByID($id);
        $this->data['recieve'] = $row->status=='received' ? $this->purchases_model->getAllPurchaserecieve($id) : array();
        // var_dump($this->data['pu']);die();
        $this->data['purchase_id'] = $id;
        $this->data['receive_no'] = $inv_no;
        $this->load->view($this->theme .'purchases/recieve', $this->data);
    }

    function purchase_recieve($id = NULL)
    {
        $this->sma->checkPermissions();
        ...
        $recieve=$this->input->post('recieve_q');
        $recieved=$this->input->post('recieved');
        $pu_itemid=$this->input->post('pu_itemid');
        $qty=$this->input->post('qty');
        $no=$this->db->query("SELECT max(no) as no FROM sma_purchase_receive")->row()->no;
        $inv_no='RC'.date('Ym-').sprintf('%04d', $no+1);
        // print_r($recieve); die();
        $purchase_row=$this->db->where('id',$id)->get('sma_purchases')->row();
        
        if($this->input->post('date')==''){
            $this->session->set_flashdata('error', "Receive Failed. Select Date to continue.");
            redirect('purchases/');
        }
        // $this->db->trans_begin();


        $date=$this->sma->fld(trim($this->input->post('date')));//date('Y-m-d H:i:s');
        ...
        $this->db->insert('sma_purchase_receive',$data);
        $recieve_id=$this->db->insert_id();
        ...
        for ($i=0; $i < count($item_id) ; $i++) { 
            $total_qty+=$qty[$i];
            $recieve_total+=($recieve[$i]+$recieved[$i]);
            $product_pu_row=$this->db->where('id',$pu_itemid[$i])->get('purchase_items')->row();
            $produt_row=$this->db->where('id',$item_id[$i])->get('products')->row();
            $amount=$product_pu_row->net_unit_cost * $recieve[$i];
            // $is_val=$this->db->query("SELECT * FROM sma_warehouses_products Where warehouse_id='$purchase_row->warehouse_id' AND product_id='".$item_id[$i]."'")->row();
            $total_recieve_amt+=$amount;
            // $pu_item_row=$this->db->query("SELECT * FROM sma_purchase_items WHERE id='".$pu_itemid[$i]."'")->row();
            // $total_receive_qty=$produt_row->quantity_balance+$recieve[$i];
            // if($total_receive_qty<=$purchase_row->quantity){
            if($recieve[$i]!=0){
                // $my_cost=$this->site->getnewavgcost($item_id[$i],$amount,$recieve[$i]);
                $detail=array('receive_id'=>$recieve_id,
                              'item_id'=>$item_id[$i],
                              'date'=>$date,
                              'trans_date'=>date('Y-m-d H:i:s'),
                              'cost'=>'',
                              'warehouse_id'=>$purchase_row->warehouse_id,
                              'purchase_itemid'=>$pu_itemid[$i],
                              'old_stock' => $this->site->getwarehouse_stock($item_id[$i],$purchase_row->warehouse_id),

                              'quantity'=>$recieve[$i]);
                // var_dump($detail);die();
                $this->db->insert('sma_purchase_receive_item',$detail);

                $stock_receive=($product_pu_row->stock_receive+$recieve[$i]);
                $this->db->query("UPDATE sma_purchase_items SET status='received',quantity_balance='".$recieve[$i]."',quantity_received='".$stock_receive."' WHERE id='".$pu_itemid[$i]."'");
        ...
        $total_t=$total_recieve_amt;//$purchase_row->total + $purchase_row->product_tax - $purchase_row->order_discount;
        if($total_qty==$recieve_total){
            $status='received';
        }
        $this->session->set_flashdata('message','Create Receive Successfull.');
        $this->db->where('id',$id)->set('status',$status)->update('sma_purchases');
