...
// ==================================add more link to print reciept===============
        $print_link=anchor('admin/sales/p_view/$1','<i class="fa fa-print"></i>'.lang('reciept_print'));
// ==========================================end================================
        ...
        $this->datatables->where('pos', 0); // ->where('sale_status !=', 'returned');
    ...
    // --------------------------------------------------------------------------------
    private function remained_of_cus($cus_id, $inv_id){
        $r = 0;
        $inv = $this->db->query('select * from sma_sales WHERE 1=1 AND id < '.$inv_id.' AND customer_id ='.$cus_id)->result();
        if(count($inv)>0){
            for($i=0;$i<count($inv); $i++){
                $r += ($inv[$i]->total - $inv[$i]->paid);
            }
        }
        return $r;
    }
    ...
    //---------------------------------------------------------------------------------
    public function p_view($id = NULL, $view = TRUE, $save_bufffer = NULL)
    {
        $this->sma->checkPermissions();
        ...
        $this->data['remained'] = $this->remained_of_cus($inv->customer_id, $inv->id);
        $this->data['customer'] = $this->site->getCompanyByID($inv->customer_id);
        $this->data['payments'] = $this->sales_model->getPaymentsForSale($id);
        $this->data['biller'] = $this->site->getCompanyByID($inv->biller_id);
        ...
        $this->data['rows'] = $this->sales_model->getAllInvoiceItems($id);
        // var_dump($this->data['rows']);die(); 
        $this->data['return_items'] = $return ? $this->sales_model->getAllReturnItems($return->id) : NULL;
        $cash=$this->sales_model->get_sum_remain_cash($inv->customer_id);
        $this->data['tot_cash']=$cash;
        $this->data['last_paid'] = $this->db->query("SELECT * FROM sma_payments WHERE sale_id = $id ORDER BY id DESC LIMIT 1")->row()->amount;
        $this->data['total_paid'] = $this->db->query("SELECT SUM(paid) as tpaid FROM sma_sales WHERE customer_id= '$inv->customer_id' AND pos<>1")->row()->tpaid;
        // echo $tot_cash->$balance;
        // print_r($cash);die();
        //$this->data['paypal'] = $this->sales_model->getPaypalSettings();
        //$this->data['skrill'] = $this->sales_model->getSkrillSettings();
        // var_dump($this->data['last_paid']);die();

        $name = lang("sale") . "_" . str_replace('/', '_', $inv->reference_no) . ".pdf";
        $html = $this->load->view($this->theme . 'sales/invoice_up', $this->data, TRUE);
        if ($view) {
            $this->load->view($this->theme . 'sales/invoice_up', $this->data);
        }
    }
// --------------------------------------end priner--------------------------
    ...
    public function suggestions()
    {
        ...
        $rows = $this->sales_model->getProductNames($sr, $warehouse_id, $category_id);
        if ($rows) {
    ...
    // SALE PAYMENT
    function sale_payment(){
        $this->data['customer'] = $this->site->getAllCompanies('customer');
        $this->data['billers'] = $this->site->getAllCompanies('biller');

        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('sales'), 'page' => lang('sales')), array('link' => '#', 'page' => lang('add_sale_payment')));
        $meta = array('page_title' => lang('add_sale_payment'), 'bc' => $bc);
        $this->page_construct('sales/list_payment', $meta, $this->data);
    }
    public function get_sale_payment(){
        // var_dump($_GET);die();
        $cus_id = $this->input->get('cus_id');
        $df =$this->input->get('date_from');
        $dt = $this->input->get('date_to');

        $cus_payment = $this->site->getAllPaymentByCus($cus_id,$df,$dt);
         // var_dump($cus_payment);die();
        $tdata='';
        $tousd =0;
        $tor = 0;
        $tob = 0;
        $i=0;
        foreach ($cus_payment as $pay) {
            $i++;
            $tdata.="<tr>";
                $tdata.="<td><input type='checkbox' class='form-control chk_sale' name='chk_sale[]' value='$pay->id' usd='".($pay->grand_total-$pay->paid)."'></td>";
                $tdata.="<td>$i</td>";
                $tdata.="<td>$pay->reference_no</td>";
                $tdata.="<td>".date('d/m/Y H:i:s',strtotime($pay->date))."</td>";
                $tdata.="<td>".number_format($pay->grand_total-$pay->paid,2)."(R)</td>";
            $tdata.="</tr>";

            $tousd +=($pay->grand_total-$pay->paid);
            
        }

        $tfoot = "<tr><th colspan='4' style='text-align:right'>Total</th>";
        $tfoot.="<th>".number_format($tousd,2)."($)</th>";
        ...
        // echo json_encode('JJJJJ');
    }
    public function save_payment(){
        $cus_id = $this->input->get('cus_id');
        $tam_usd  = $this->input->get('amount_usd');
        // $tam_r = $this->input->get('amount_r');
        // $tam_b = $this->input->get('amount_b');
        $df =$this->input->get('date_from');
        $dt = $this->input->get('date_to');
        $sale_ids = $this->input->get('sale_ids');
        $amount_usd = 0;
        $date = date('Y-m-d');
        ...
        $sales_pay = $this->site->get_selected_sale($cus_id,$sale_ids);
        ...
        $rec_id = $this->sales_model->save_payment_record($datarec);
        // print_r($rec_id);die();
        foreach ($sales_pay as $val) {
            ...
            // $amount_b = 0;
            $sale_id = $val->id;
            // $balance_r = ($val->grand_r - $val->paid_r);
            $balance_usd = ($val->grand_total - $val->paid);
            // $balance_b = ($val->grand_b - $val->paid_b);
            ...
            // if ($tam_r<$balance_r && $tam_r>0) {

            //     $amount_r = $tam_r;
            // }
            // if ($tam_b<$balance_b && $tam_b>0) {
            ...
            $payment = array(
                ...
                // 'amount_r'=>$this->sma->formatDecimal($amount_r),
                // 'amount_b'=>$this->sma->formatDecimal($amount_b),
                'paid_by' =>'cash',
                'cc_type' => 'Visa',
            ...
            if($this->sales_model->addPayment($payment)){
                ...
                $this->sales_model->save_record_detail($recdetail);
    ...
    function list_sale_payment(){
        $this->data['record'] = $this->site->getRecordpayment();
        // var_dump($this->data['record']);die();
        // $this->data['remain'] = $this->site->get_amount_remain();
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('sales'), 'page' => lang('sales')), array('link' => '#', 'page' => lang('list_sale_payment')));
        $meta = array('page_title' => lang('list_sale_payment'), 'bc' => $bc);
        $this->page_construct('sales/list_sale_payment', $meta, $this->data);
    }
    //  private function total_remained($cus_id, $inv_id){
    //     $r = 0;
    //     $inv = $this->db->query('select * from sma_sales WHERE 1=1 AND id < '.$inv_id.' AND customer_id ='.$cus_id)->result();
    //     if(count($inv)>0){
    //         for($i=0;$i<count($inv); $i++){
    ...
    function print_payment($id)
    {
        ...
        // $this->data['inv']=$this->sales_model->getInvoiceByID($id);
        // $inv=$this->data['inv'];
        // $this->data['get_remain']=$this->db->query("SELECT sum(grand_total)  from sma_sales where customer_id= '$inv->customer_id' AND payment_status<>'paid'")->result();
// =============================================||==================================================
        // $this->data['last_paid'] = $this->db->query("SELECT * FROM sma_payments WHERE sale_id = $id ORDER BY id DESC LIMIT 1")->row()->amount;
        // $this->data['total_paid'] = $this->db->query("SELECT SUM(paid) as tpaid FROM sma_sales WHERE customer_id= '$inv->customer_id' AND pos<>1")->row()->tpaid;
// =============================================||==================================================
        // var_dump($this->data['inv']);die();

        $this->data['rate']=$this->site->getrielcurrency();
        $this->data['voucher'] = $this->site->getRecordpaymentByid($id);
        $this->data['biller'] = $this->site->getCompanyByID($this->data['voucher']->billers);
        $this->data['balance'] = $this->site->getCusSale($this->data['voucher']->cus_id);

        $this->data['voucher_detail'] = $this->site->get_RecDetail($id);
        // $this->data['remain'] = $this->site->get_remain_payment($id);
        // var_dump($this->data['balance']);die();
        $this->load->view($this->theme . 'sales/print_pay', $this->data);
    }
    function list_suspend(){
        $this->data['suspends']=$this->db->query("SELECT sma_suspended_bills.date,sma_suspended_bills.customer,SUM(sma_suspended_items.subtotal) as total,sma_users.username as username,sma_users.phone as phone FROM sma_suspended_bills
            LEFT JOIN sma_users
            ON sma_users.id=sma_suspended_bills.created_by
            LEFT JOIN sma_suspended_items
            ON sma_suspended_bills.id=sma_suspended_items.suspend_id
            GROUP BY sma_suspended_items.suspend_id")->result();
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('sales'), 'page' => lang('sales')), array('link' => '#', 'page' => lang('list_sale_payment')));
        $meta = array('page_title' => lang('list_suspend'), 'bc' => $bc);
        $this->page_construct('sales/list_suspend', $meta, $this->data);
    }
