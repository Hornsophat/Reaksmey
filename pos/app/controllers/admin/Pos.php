...
require 'assets/php_print/autoload.php';
...
class Pos extends MY_Controller
{
    ...
    public function index($sid = NULL)
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $hotel_checkin_id = $this->input->post('checkin_id');
            $cus_name = $this->input->post('customer_name');
            $cus_phone = $this->input->post('customer_phone');
            $cus_company = $this->input->post('customer_company');
            // $cus_name = $this->db->query("SELECT Family FROM ci_samnak_neak.tbl_customer WHERE id ='".$cushotel_id."'")->row()->Family;
            // var_dump($cus_name);die();
            if ($cus_phone != '') {
                $newCustomer = array(
                                'group_id'      => 3,
                                'group_name'    => 'customer',
                                ...
                                'logo'          => 'logo.png',
                                'award_points'  => 0,
                                'cusHotel_id'   => $cushotel_id,
                                // 'identity'      => 0,
                                // 'swarehouse_id' => 0,
                            );
                // var_dump($newCustomer);die();
                $dublicate_val = $this->db->query("SELECT phone FROM sma_companies WHERE phone <> '' AND group_name = 'customer'")->result();
                foreach ($dublicate_val as $dprow) {
                    $dublicate = $dprow->phone;
                }
                if ($dublicate == $cus_phone) {
                        $dbcus_id = $this->db->query("SELECT id as cus_id FROM sma_companies WHERE phone = '".$cus_phone."' AND group_name ='customer'")->row()->cus_id;
                        $customer_id = $dbcus_id;
                    }else{
                        $this->db->insert('sma_companies',$newCustomer);
                        $cus_insertid = $this->db->insert_id();
                        if ($cus_insertid != '') {
                            $customer_id = $cus_insertid;
                        }else{
            ...
            $customer = $customer_details->name != '-'  ? $customer_details->name : $customer_details->name; //change from company to name
            $biller_details = $this->site->getCompanyByID($biller_id);
            $biller = $biller_details->company != '-' ? $biller_details->company : $biller_details->name;
            $note = $this->sma->clear_tags($this->input->post('pos_note'));
            $staff_note = $this->sma->clear_tags($this->input->post('staff_note'));
            $reference = $this->site->getReference('pos');
            $table_id = $this->input->post('customer');
            ...
            $data = array('date'              => $date,
                          ...
                          'sub_tableid'       => $table_id,
                            'hotel_checkin_id' => $hotel_checkin_id,
        ...
        if ($this->form_validation->run() == TRUE && !empty($products) && !empty($data)) {
            if ($suspend) {
                if ($this->pos_model->suspendSale($data, $products, $did, $customer_id)) {
            ...
            if (($sid || $duplicate_sale) && $inv_items) {
                    ...
                    foreach ($inv_items as $item) {
                        ...
                        $unit = $this->db->where('id',$row->unit)->get('units')->row();
            ...
            $this->data['floors'] = $this->getFloor();
            $this->data['cus_new'] = $this->site->getHotelCustomer();
            $this->data['HotelRooms'] = $this->site->getHotelCheckinRooms();

            // var_dump($this->data['cus_new']);die();
            ...
            $this->data['munit']=$this->db->get('units')->result();
            $this->load->view($this->theme . 'pos/add', $this->data);
        }
    }
    function getFloor(){
        $sql = $this->db->query("SELECT tbl_location FROM sma_companies WHERE group_name = 'table' AND tbl_location <> '' GROUP BY tbl_location")->result();
        return $sql;
    }

    function phone_con($sid = null){
        $this->sma->checkPermissions();

                 $this->load->library('pagination');


         $this->data['cate']=$this->get_cat_menu();

        $config=array();
        $config["base_url"]= admin_url().'pos/phone_con/';
        $config["total_rows"]=$this->count_customer_pagination();
        $config["per_page"]=30;
        ...
        // ===============================================note call id from other method to use this===================================================
        $data = $this->get_cat_menu();
        $pro = array();
        foreach ($data as $row) {
            $pro[$row->id] = $this->get_contan_menu($row->id,$config["per_page"],$this->uri->segment(3));
        }
        $this->data['product_query'] = $pro;//$this->get_contan_menu($config["per_page"],$this->uri->segment(4));
        $this->data['link']=$this->pagination->create_links();
         // $this->data['product_query'] = $this->get_contan_menu();
         // var_dump($this->data['product_query']);die();
        ...
        if ($this->form_validation->run() == TRUE && !empty($products) && !empty($data)) {
            ...
            $this->load->view($this->theme . 'pos/order_view', $this->data);
        }
    }
    ...
    public function get_contan_menu($cate_id,$limit,$offset){
        $this->db->limit($limit,$offset);
        $this->db->where('category_id',$cate_id);
        return $query=$this->db->get("sma_products")->result();
        
    }

    public function count_customer_pagination(){
        return $this->db->count_all('sma_products');
    }
    ...
    public function view_phone(){
        ...
        $config=array();
        $config["base_url"]= admin_url().'pos/view_phone/';
        $config["total_rows"]=$this->count_customer_pagination();
        $config["per_page"]=30;
        $config['prev_link']=false;
        $config['next_link']=false;
        ...
        // ===============================================note call id from other method to use this===================================================
         ...
         $pub_tax=0;
        if($this->input->post('is_pub_val')==1){
            $pub_tax=0.03;
        }
        ...
        if ($this->form_validation->run() == true) {
            ...
            $biller_details = $this->site->getCompanyByIDs($biller_id);
            ...
            $total_fianl=0;
            $pub_taxval=$total*$pub_tax;
            $total_fianl=$this->sma->formatDecimal($total+$pub_taxval);
            if ($this->input->post('discount')) {
                $order_discount_id = $this->input->post('discount');
                $opos = strpos($order_discount_id, $percentage);
                if ($opos !== false) {
                    $ods = explode("%", $order_discount_id);
                    $order_discount = $this->sma->formatDecimal((($total_fianl + $product_tax) * (Float)($ods[0])) / 100);
                } else {
                    $order_discount = $this->sma->formatDecimal($order_discount_id);
                }
            } else {
            ...
            if ($this->Settings->tax2) {
                ...
                if ($order_tax_details = $this->site->getTaxRateByID($order_tax_id)) {
                    ...
                    if ($order_tax_details->type == 1) {
                        $order_tax = $this->sma->formatDecimal((($total_fianl + $product_tax - $order_discount) * $order_tax_details->rate) / 100);
                    }
                }
            } else {
                $order_tax_id = NULL;
            ...
            $grand_total = $this->sma->formatDecimal($this->sma->formatDecimal($total_fianl) + $total_tax + $this->sma->formatDecimal($shipping) - $order_discount);
            $data = array('date' => $date,
                'reference_no' => $reference,
                'customer_id' => $customer_id,
                'customer' => $customer,
                ...
                'paid' => $this->input->post('amount-paid') ? $this->input->post('amount-paid') : 0,
                'created_by' => $this->session->userdata('user_id'),
                'exchange_rate' => $this->input->post('ex_rate'),
                'is_pub_tax' => $this->input->post('is_pub_val')
            );
        ...
        if ($this->form_validation->run() == true && !empty($products) && !empty($data)) {
          if ($suspend) {
            ...
            $this->data["tcp"] = $this->pos_model->products_count($this->pos_settings->default_category);
            $this->data['products'] = $this->ajaxproducts($this->pos_settings->default_category);
            $this->data['tables'] = $this->site->getAllCompanies('customer'); // get tables
            $this->data['categories'] = $this->site->getAllCategories();
            // $this->data['subcategories'] = $this->pos_model->getSubCategoriesByCategoryID($this->pos_settings->default_category);
            $this->data['pos_settings'] = $this->pos_settings;
            // $this->data['exch'] = $this->pos_model->getexchangerate();

           // var_dump($this->data['tables']);die();

           // $this->data['cat']=$this->get_cate();
    ...
    public function view($sale_id = NULL, $modal = NULL)
    {
        ...
        $this->data['rate']=$this->pos_model->getrielcurrency();
        // print_r($this->data['rate']);die();
        ...
        if($modal==1){
             ...
             if ($u_re[0]->print_receipt == 1) { 
               $this->load->view($this->theme . 'pos/up_view', $this->data);
            }else{
                $this->load->view($this->theme . 'pos/invoice_up', $this->data);
            }
        }
    ...
    function get_all_user_print($u_id){
        $q="SELECT * from sma_users where id='$u_id'";
        return $this->db->query($q)->result();
    }

    function get_unit($id){
        $re="SELECT * FROM sma_units where id=$id";
        return $this->db->query($re)->row();
    }
 ...
 public function opened_bills($per_page = 0)
    {
        ...
        if (!empty($bills)) {
            ...
            foreach ($bills as $bill) {
                $total = $this->db->select_sum('subtotal', 'subtotal')->where('suspend_id', $bill->id)->get('suspended_items')->row();
                if ($this->Owner || $this->Admin) {
                    $html .= '<li class="all-sus" id="sus_'.$bill->id.'"><div class="del-sus" id="'. $bill->id .'"><i class="fa fa-close"></i></div>';
                }else{
                $html .= '<li class="all-sus" id="sus_'.$bill->id.'">';
                }
                $html .= '<button type="button" class="btn btn-info sus_sale" id="' . $bill->id . '">';
                $html .= '<p class="hide">' . $bill->suspend_note . '</p>
                          <strong>' . $bill->customer . '</strong>
                          <p class="hide">Date: ' . $bill->date . '</p>
                          <p class="hide">Items: ' . $bill->count . '</p>
                          <p class="" style="font-size:11px !important;">Total: $' . $this->sma->formatMoney($total->subtotal) . '</p>';

                $html .= '</button>';
                $html .= '<div class="control-button">
                          <button type="button" class="btn btn-warning btn-sm pay_sus" id="' . $bill->id . '" title="Payment"><i class="fa fa-money"></i></button>
                          </div>';
                $html .= '</li>';
            }
            $html .= '</ul>';
    ...
    function getTransfer(){
        $this->data['title'] = 'Transfer Food';
        $this->data['item_sus'] = $this->getAllbillForTransfer();
        // var_dump($this->data['item_sus']);die();
        echo $this->load->view($this->theme . 'pos/transfer_food',$this->data);
    }

    function getAllbillForTransfer(){
        $sql = $this->db->query("SELECT * FROM sma_suspended_bills")->result();
        return $sql;
    }

    function opened_bills_table($per_page = 0){
        $this->load->library('pagination');
        ...
        $config['base_url'] = admin_url('pos/opened_bills_table');
        $config['total_rows'] = $this->pos_model->bills_count();
        $config['per_page'] = 6;
        $config['num_links'] = 3;
        ...
        if (!empty($bills)) {
            ...
            $html .= '<ul class="ob">';
            foreach ($bills as $bill) {
                $html .= '<li><button type="button" class="btn btn-info suss_sale" id="' . $bill->id . '"><p>' . $bill->suspend_note . '</p><strong>' . $bill->customer . '</strong><br>'.lang('date').': ' . $bill->date . '<br>'.lang('items').': ' . $bill->count . '<br>'.lang('total').': ' . $this->sma->formatMoney($bill->total) . '</button></li>';
            }
            $html .= '</ul>';
        } else {
            $html = "<h3>" . lang('no_opeded_bill') . "</h3><p>&nbsp;</p>";
        ...
        $data['page'] = $this->pagination->create_links();
        echo $this->load->view($this->theme . 'pos/opened_table', $data, TRUE);
    }
    ...
    function save_receipt(){
        ...
        $savefile = @file_put_contents("assets/php_print/output/rec.jpg", base64_decode(explode(",", $_POST['data'])[1]));
    ...
    function save_receiptsec(){
        $savefiles = @file_put_contents("assets/php_print/secoutput/recsec.jpg", base64_decode(explode(",", $_POST['data'])[1]));
        //if the file saved properly, print the file name
        if($savefiles){
            echo 1;
        }else{
    ...
    function do_print_receipt(){
         ...
         */
          // $source = __DIR__ . "/../resources/document_test.php";   D:\xampp\htdocs\assets\php_print\output
          //  echo $source;die();
        try {
            /* Set up command */
            $source = FCPATH."assets/php_print/output/rec.jpg";

            // $source = "E:\wampserver\www\Restuarant\kps_restaurant\assets\php_print\output/rec.jpg";
            /* Load up the image */
            // var_dump($source);die();
            try{
    ...
    function do_print_receiptsec(){
        ...
        try {
            /* Set up command */
            $source = FCPATH."assets/php_print/secoutput/recsec.jpg";
    ...
    function get_transferTable(){
        $f_id = $_GET['from_id'];
        $t_id = $_GET['too_id'];

        $t_result = $this->pos_model->get_updateTransfer($f_id,$t_id);
        if ($t_result) {
    ...
    function getTableByFloor(){
        $fl = $_GET['val'];
        $data = $this->site->getAllTables($fl);
        $fl_table = array();
        foreach ($data as $row) {
            $fl_table[$row->id] = $row->name;
        }
        $data['floor_table'] = $fl_table;
        echo json_encode($data);
    }

    function getHotelContactDetail($cus_id){
        $data = $this->site->getHotelCustomerById($cus_id);
        // var_dump($data);die();
        echo json_encode($data);
    }
